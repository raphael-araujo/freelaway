...
def encontrar_jobs(request):
    if request.method == 'GET':
        ...
        if preco_minimo or preco_maximo or prazo_minimo or prazo_maximo or categoria:
            ...
            jobs = Job.objects.filter(preco__gte=preco_minimo).filter(preco__lte=preco_maximo).filter(
                prazo_entrega__gte=prazo_minimo).filter(prazo_entrega__lte=prazo_maximo).filter(
                categoria__in=categoria).filter(reservado=False)
